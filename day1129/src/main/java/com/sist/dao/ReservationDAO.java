...
public class ReservationDAO {
	...
	public ArrayList<PossibleReservationRoomVO> listPossibleReservationRoom(){
		...
		try {
			...
			while(rs.next()) {
				list.add(new PossibleReservationRoomVO(rs.getInt(1), rs.getInt(2), 
	...
	public String getReservationNo() {
		...
		int last  = 0;
		String sql = "select nvl(max(substr(reservation_no,9)), 0) + 1 from reservation "
				+ "where substr(reservation_no,1,8) = to_char(sysdate,'yyyymmdd')";
		try {
			Connection conn = ConnectionProvider.getConnection();
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery(sql);
		...
		System.out.println("새로운 예약번호:"+no);
		return no;
	}
	...
	public void updateTotal(String reservation_no) {
		...
		String sql = "update reservation set total = (select fee*(select checkout-checkin from reservation "
				+ "where reservation_no = ?) from room where room_no = (select room_no from "
				+ "reservation where reservation_no=?)) "
