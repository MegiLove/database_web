...
public class ReservationDAO {
	public ArrayList<PossibleReservationRoomVO> listPossibleReservation(){
		ArrayList<PossibleReservationRoomVO> list= new ArrayList<PossibleReservationRoomVO>();
		String sql="select room_no, fee, room_type_name,room_state_name from room r, room_type t , room_state s where r.room_type_code=t.room_type_code and "
	...
	public int insertReservation(ReservationVO r) {
		int re=-1;
		String sql="insert into reservation(reservation_no,checkin,checkout,phone,total,count_guest,customer_no,room_no,eno) values(?,?,?,?,0,?,?,?,1000)";
		...
		pstmt.setInt(6, r.getCount_guest());
		pstmt.setInt(7, r.getCustomer_no());
		pstmt.setInt(8, r.getRoom_no());
		return re;
	}
}
